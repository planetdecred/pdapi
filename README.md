...
[![Build Status](https://github.com/planetdecred/pdapi/workflows/Build%20and%20Test/badge.svg)](https://github.com/planetdecred/pdapi/actions)
[![Latest tag](https://img.shields.io/github/tag/planetdecred/pdapi.svg)](https://github.com/planetdecred/pdapi/tags)
[![Go Report Card](https://goreportcard.com/badge/github.com/planetdecred/pdapi)](https://goreportcard.com/report/github.com/planetdecred/pdapi)
[![ISC License](https://img.shields.io/badge/license-ISC-blue.svg)](http://copyfree.org)

## Overview

pdapi is an original [Decred](https://www.decred.org/) block explorer, with
...
- [pdapi](#pdapi)
  ...
  - [Building](#building)
    ...
    - [Building pdapi with Go](#building-pdapi-with-go)
  ...
  - [Getting Started](#getting-started)
    ...
    - [Creating the pdapi Configuration File](#creating-the-pdapi-configuration-file)
  ...
  - [System Hardware Requirements](#system-hardware-requirements)
    - [pdapi only (PostgreSQL on other host)](#pdapi-only-postgresql-on-other-host)
    - [pdapi and PostgreSQL on same host](#pdapi-and-postgresql-on-same-host)
  - [pdapi Daemon](#pdapi-daemon)
...
| Current     | 5.3     | `5.3-stable`  | `v5.3.0`           | same as pdapi 5.2                |
| Legacy      | 5.2     | `5.2-stable`  | `v5.2.2`           | ^6.1.1 (dcrd v1.6 release)         |

## Repository Overview

```none
../pdapi                The main Go MODULE. See cmd/pdapi for the explorer executable.
├── api/types             The exported structures used by the pdapi and Insight APIs.
├── blockdata             Package blockdata is the primary data collection and
|                           storage hub, and chain monitor.
├── cmd
│   ├── pdapi           MODULE for the pdapi explorer executable.
│   │   ├── api           pdapi's own HTTP API
...
│   |                       data to multiple clients like pdapi.
...
- Running `dcrd` running with `--txindex --addrindex`, and synchronized to the
  current best block on the network. On startup, pdapi will verify that the
...
documentation](docs/docker.md) for more information. The supported pdapi build
instructions are described below.

## Building

The pdapi build process comprises two general steps:

1. Bundle the static web page assets with Webpack (via the `npm` tool).
2. Build the `pdapi` executable from the Go source files.
...
- Clone the pdapi repository. It is conventional to put it under `GOPATH`, but
  this is no longer necessary (or recommend) with Go modules. For example:

  ```sh
  git clone https://github.com/planetdecred/pdapi $HOME/go-work/github/planetdecred/pdapi
  ```

Note that none of the above is required at runtime.


### Building pdapi with Go

Change to the `cmd/pdapi` folder and build:
...
`pdapi version 5.1.0-pre+dev (Go version go1.12.7)`.  However, it may be
...
This produces a string like `pdapi version 6.0.0-beta+750fd6c2 (Go version go1.16.2)`.
...
folders to ensure that _the user running pdapi_ is able to access these paths.
...
```sh
cd $HOME/go-work/github/planetdecred/pdapi
...
No special actions are required. Simply start the new pdapi and automatic
database schema upgrades and table data patches will begin.

### From v2.x or earlier

The database scheme change from pdapi v2.x to v3.x does not permit an
automatic migration. The tables must be rebuilt from scratch:

1. Drop the old pdapi database, and create a new empty pdapi database.

   ```sql
   -- Drop the old database.
   DROP DATABASE pdapi;

   -- Create a new database with the same "pguser" set in the pdapi.conf.
   CREATE DATABASE pdapi OWNER pdapi;
   ```

2. Delete the pdapi data folder (i.e. corresponding to the `datadir` setting).
   By default, `datadir` is in `{appdata}/data`:

   - Linux: `~/.pdapi/data`
   - Mac: `~/Library/Application Support/Pdapi/data`
   - Windows: `C:\Users\<your-username>\AppData\Local\Pdapi\data` (`%localappdata%\Pdapi\data`)

3. With dcrd synchronized to the network's best block, start pdapi to begin
...
subtract 1.5-2GB from your system RAM so pdapi itself will have plenty of
...
the RAM available to the pdapi process, which will increase as request
volume increases and its cache becomes fully utilized. Allocate sufficient
memory to pdapi for your application, and use a reverse proxy such as nginx
...
While pdapi has some special handling for [CockroachDB](https://github.com/planetdecred/pdapi/issues/1291),
...
- Compared to a well-configure PostgreSQL backend, CoackroachDB performance is
  suboptimal. See the [CockroachDB issue](https://github.com/planetdecred/pdapi/issues/1291)
  for more information.
- The bulk of the testing and performance optimization is done with PostgreSQL
  in mind.

If you decide to use CockroachDB with pdapi, (1) do not do so in production
and (2) expect some bugs and relatively poor performance.

See [pdapi's CockroachDB wiki page](https://github.com/planetdecred/pdapi/wiki/CockroachDB)
for more information.

### Creating the pdapi Configuration File

Begin with the sample configuration file. With the default `appdata` directory
for the current user on Linux:

```sh
cp sample-pdapi.conf ~/.pdapi/pdapi.conf
```

Then edit pdapi.conf with your dcrd RPC settings. See the output of `pdapi
--help` for a list of all options and their default values.

### Indexing the Blockchain

If pdapi has not previously been run with the PostgreSQL database backend, it
is necessary to perform a bulk import of blockchain data and generate table
indexes. _This will be done automatically by `pdapi`_ on a fresh startup.
**Do NO interrupt the initial sync or use the browser interface until it is
completed.**

Note that pdapi requires that
...
If these parameters are not set, pdapi will be unable to retrieve transaction
details and perform address searches, and will exit with an error mentioning
these indexes.

### Starting pdapi

Launch the pdapi daemon and allow the databases to process new blocks.
...
On subsequent launches, only blocks new to pdapi are processed.

```sh
./pdapi    # don't forget to configure pdapi.conf in the appdata folder!
```
...
initial sync can leave pdapi and it's databases in an unrecoverable or
...
### pdapi only (PostgreSQL on other host)
...
### pdapi and PostgreSQL on same host

These specifications assume pdapi and postgres are running on the same machine.
...
The `cmd/pdapi` folder contains the `main` package for the `pdapi` app, which
...
After pdapi syncs with the blockchain server via RPC, by default it will begin
...
Note that while pdapi can be started with HTTPS support, it is recommended to
employ a reverse proxy such as Nginx ("engine x"). See sample-nginx.conf for an
example Nginx configuration.

## APIs

The pdapi block explorer is exposed by two APIs: a Decred implementation of
...
pdapi API uses the path prefix `/api`.
...
The pdapi API is a REST API accessible via HTTP. To call the pdapi API, use
the `/api` path prefix.
...
From the cmd/pdapi directory, run the following command to install the node
modules.
...
The core functionality of pdapi is server-side rendered in Go and designed to
...
`killall -USR1 pdapi` from the command line.
...
The core functionality of pdapi should perform well in low power device / high
...
use by other pdapi packages, but may be of general value in
the future.
...
See the GitHub [issue trackers](https://github.com/planetdecred/pdapi/issues) and
the [project milestones](https://github.com/planetdecred/pdapi/milestones).
...
5. Create a [pull request](https://github.com/planetdecred/pdapi/compare).
