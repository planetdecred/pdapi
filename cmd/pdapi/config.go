...
import (
	...
	"github.com/planetdecred/pdapi/v6/netparams"
)

const (
	defaultConfigFilename = "pdapi.conf"
	defaultLogFilename    = "pdapi.log"
...
var (
	defaultHomeDir           = dcrutil.AppDataDir("pdapi", false)
	...
	defaultMainnetLink  = "https://explorer.pdapi.org/"
	defaultTestnetLink  = "https://testnet.pdapi.org/"
	defaultOnionAddress = ""
...
type config struct {
	...
	HomeDir      string `short:"A" long:"appdata" description:"Path to application home directory" env:"PDAPI_APPDATA_DIR"`
	ConfigFile   string `short:"C" long:"configfile" description:"Path to configuration file" env:"PDAPI_CONFIG_FILE"`
	DataDir      string `short:"b" long:"datadir" description:"Directory to store data" env:"PDAPI_DATA_DIR"`
	LogDir       string `long:"logdir" description:"Directory to log output." env:"PDAPI_LOG_DIR"`
	...
	OutFolder    string `short:"f" long:"outfolder" description:"Folder for file outputs" env:"PDAPI_OUT_FOLDER"`
	ShowVersion  bool   `short:"V" long:"version" description:"Display version information and exit"`
	TestNet      bool   `long:"testnet" description:"Use the test network (default mainnet)" env:"PDAPI_USE_TESTNET"`
	SimNet       bool   `long:"simnet" description:"Use the simulation test network (default mainnet)" env:"PDAPI_USE_SIMNET"`
	DebugLevel   string `short:"d" long:"debuglevel" description:"Logging level {trace, debug, info, warn, error, critical}" env:"PDAPI_LOG_LEVEL"`
	Quiet        bool   `short:"q" long:"quiet" description:"Easy way to set debuglevel to error" env:"PDAPI_QUIET"`
	HTTPProfile  bool   `long:"httpprof" short:"p" description:"Start HTTP profiler." env:"PDAPI_ENABLE_HTTP_PROFILER"`
	HTTPProfPath string `long:"httpprofprefix" description:"URL path prefix for the HTTP profiler." env:"PDAPI_HTTP_PROFILER_PREFIX"`
	CPUProfile   string `long:"cpuprofile" description:"File for CPU profiling." env:"PDAPI_CPU_PROFILER_FILE"`
	UseGops      bool   `short:"g" long:"gops" description:"Run with gops diagnostics agent listening. See github.com/google/gops for more information." env:"PDAPI_USE_GOPS"`

	// API/server
	APIProto            string  `long:"apiproto" description:"Protocol for API (http or https)" env:"PDAPI_ENABLE_HTTPS"`
	APIListen           string  `long:"apilisten" description:"Listen address for API. default localhost:7777, :17778 testnet, :17779 simnet" env:"PDAPI_LISTEN_URL"`
	...
	UseRealIP           bool    `long:"userealip" description:"Use the RealIP middleware from the pressly/chi/middleware package to get the client's real IP from the X-Forwarded-For or X-Real-IP headers, in that order." env:"PDAPI_USE_REAL_IP"`
	CacheControlMaxAge  int     `long:"cachecontrol-maxage" description:"Set CacheControl in the HTTP response header to a value in seconds for clients to cache the response. This applies only to FileServer routes." env:"PDAPI_MAX_CACHE_AGE"`
	InsightReqRateLimit float64 `long:"insight-limit-rps" description:"Requests/second per client IP for the Insight API's rate limiter." env:"PDAPI_INSIGHT_RATE_LIMIT"`
	...
	// Mempool
	MempoolMinInterval int `long:"mp-min-interval" description:"The minimum time in seconds between mempool reports, regardless of number of new tickets seen." env:"PDAPI_MEMPOOL_MIN_INTERVAL"`
	MempoolMaxInterval int `long:"mp-max-interval" description:"The maximum time in seconds between mempool reports (within a couple seconds), regardless of number of new tickets seen." env:"PDAPI_MEMPOOL_MAX_INTERVAL"`
	MPTriggerTickets   int `long:"mp-ticket-trigger" description:"The minimum number of new tickets that must be seen to trigger a new mempool report." env:"PDAPI_MP_TRIGGER_TICKETS"`

	// Politeia/proposals and consensus agendas
	AgendasDBFileName string `long:"agendadbfile" description:"Agendas DB file name (default is agendas.db)." env:"PDAPI_AGENDAS_DB_FILE_NAME"`
	ProposalsFileName string `long:"proposalsdbfile" description:"Proposals DB file name (default is proposals.db)." env:"PDAPI_PROPOSALS_DB_FILE_NAME"`
	...
	NoDevPrefetch    bool   `long:"no-dev-prefetch" description:"Disable automatic dev fund balance query on new blocks. When true, the query will still be run on demand, but not automatically after new blocks are connected." env:"PDAPI_DISABLE_DEV_PREFETCH"`
	...
	// DB backend
	PGDBName         string        `long:"pgdbname" description:"PostgreSQL DB name." env:"PDAPI_PG_DB_NAME"`
	PGUser           string        `long:"pguser" description:"PostgreSQL DB user." env:"PDAPI_POSTGRES_USER"`
	PGPass           string        `long:"pgpass" description:"PostgreSQL DB password." env:"PDAPI_POSTGRES_PASS"`
	PGHost           string        `long:"pghost" description:"PostgreSQL server host:port or UNIX socket (e.g. /run/postgresql)." env:"PDAPI_POSTGRES_HOST_URL"`
	...
	SyncAndQuit      bool          `long:"sync-and-quit" description:"Sync to the best block and exit. Do not start the explorer or API." env:"PDAPI_ENABLE_SYNC_N_QUIT"`
	ImportSideChains bool          `long:"import-side-chains" description:"(experimental) Enable startup import of side chains retrieved from dcrd via getchaintips." env:"PDAPI_IMPORT_SIDE_CHAINS"`
	...
	// RPC client options
	DcrdUser         string `long:"dcrduser" description:"Daemon RPC user name" env:"PDAPI_DCRD_USER"`
	DcrdPass         string `long:"dcrdpass" description:"Daemon RPC password" env:"PDAPI_DCRD_PASS"`
	DcrdServ         string `long:"dcrdserv" description:"Hostname/IP and port of dcrd RPC server to connect to (default localhost:9109, testnet: localhost:19109, simnet: localhost:19556)" env:"PDAPI_DCRD_URL"`
	DcrdCert         string `long:"dcrdcert" description:"File containing the dcrd certificate file" env:"PDAPI_DCRD_CERT"`
	DisableDaemonTLS bool   `long:"nodaemontls" description:"Disable TLS for the daemon RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost" env:"PDAPI_DCRD_DISABLE_TLS"`
	...
	EnableExchangeBot bool   `long:"exchange-monitor" description:"Enable the exchange monitor" env:"PDAPI_MONITOR_EXCHANGES"`
	DisabledExchanges string `long:"disable-exchange" description:"Exchanges to disable. See /exchanges/exchanges.go for available exchanges. Use a comma to separate multiple exchanges" env:"PDAPI_DISABLE_EXCHANGES"`
	ExchangeCurrency  string `long:"exchange-currency" description:"The default bitcoin price index. A 3-letter currency code" env:"PDAPI_EXCHANGE_INDEX"`
	RateMaster        string `long:"ratemaster" description:"The address of a DCRRates instance. Exchange monitoring will get all data from a DCRRates subscription." env:"PDAPI_RATE_MASTER"`
	RateCertificate   string `long:"ratecert" description:"File containing DCRRates TLS certificate file." env:"PDAPI_RATE_MASTER"`

	// Links
	MainnetLink  string `long:"mainnet-link" description:"When pdapi is on testnet, this address will be used to direct a user to a pdapi on mainnet when appropriate." env:"PDAPI_MAINNET_LINK"`
	TestnetLink  string `long:"testnet-link" description:"When pdapi is on mainnet, this address will be used to direct a user to a pdapi on testnet when appropriate." env:"PDAPI_TESTNET_LINK"`
	OnionAddress string `long:"onion-address" description:"Hidden service address" env:"PDAPI_ONION_ADDRESS"`
}
