...
func TestMain(m *testing.M) {
	...
	tempConfigFile, _ = ioutil.TempFile("", "pdapi_test_file.cfg")
	defer os.Remove(tempConfigFile.Name())
	os.Setenv("PDAPI_CONFIG_FILE", tempConfigFile.Name())

	// Make an empty folder for appdata tests.
	tempAppDataDir, _ = ioutil.TempDir("", "pdapi_test_appdata")
	...
	retCode := m.Run()
	os.Unsetenv("PDAPI_CONFIG_FILE")

	os.Exit(retCode)
}

// disableConfigFileEnv checks if the PDAPI_CONFIG_FILE environment variable
// is set, unsets it, and returns a function that will return
// PDAPI_CONFIG_FILE to its state before calling disableConfigFileEnv.
func disableConfigFileEnv() func() {
	loc, wasSet := os.LookupEnv("PDAPI_CONFIG_FILE")
	if wasSet {
		os.Unsetenv("PDAPI_CONFIG_FILE")
		return func() { os.Setenv("PDAPI_CONFIG_FILE", loc) }
	}
	return func() {}
}
...
func TestLoadCustomConfigPresent(t *testing.T) {
	...
	_, err := loadConfig()
	if err != nil {
		t.Fatalf("Failed to load pdapi config: %v", err)
	}
}
...
func TestLoadDefaultConfigMissing(t *testing.T) {
	...
	os.Setenv("PDAPI_APPDATA_DIR", tempAppDataDir)
	defer os.Unsetenv("PDAPI_APPDATA_DIR")
...
func TestLoadCustomConfigMissing(t *testing.T) {
	...
	os.Setenv("PDAPI_CONFIG_FILE", goneFile.Name())

	// Attempt to load using the non-existent non-default config file, which
	// should return an error.
	_, err := loadConfig()
	if err == nil {
		t.Errorf("Loaded pdapi config, but the explicitly set config file"+
...
func TestDefaultConfigAPIListenWithEnv(t *testing.T) {
	...
	os.Setenv("PDAPI_LISTEN_URL", customListenPath)
...
func TestDefaultConfigAppDataDir(t *testing.T) {
	expected := dcrutil.AppDataDir("pdapi", false)
	cfg, err := loadConfig()
	if err != nil {
		t.Fatalf("Failed to load pdapi config: %v", err)
	}
...
func TestDefaultConfigHomeDirWithEnvAndFlag(t *testing.T) {
	...
	os.Setenv("PDAPI_APPDATA_DIR", cliOverride)
	defer os.Unsetenv("PDAPI_APPDATA_DIR")
...
func TestDefaultConfigTestNetWithEnv(t *testing.T) {
	os.Setenv("PDAPI_USE_TESTNET", "true")
	defer os.Unsetenv("PDAPI_USE_TESTNET")
...
func TestDefaultConfigTestNetWithEnvAndBadValue(t *testing.T) {
	os.Setenv("PDAPI_USE_TESTNET", "no")
	defer os.Unsetenv("PDAPI_USE_TESTNET")

	_, err := loadConfig()
	if err == nil {
		t.Errorf("Invalid boolean value for PDAPI_USE_TESTNET did not cause an error.")
	}
}
