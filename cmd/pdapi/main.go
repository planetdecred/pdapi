...
import (
	...
	"github.com/planetdecred/pdapi/db/dcrpg/v6"
	"github.com/planetdecred/pdapi/exchanges/v3"
	"github.com/planetdecred/pdapi/gov/v4/agendas"
	"github.com/planetdecred/pdapi/gov/v4/politeia"

	"github.com/planetdecred/pdapi/v6/blockdata"
	"github.com/planetdecred/pdapi/v6/db/cache"
	"github.com/planetdecred/pdapi/v6/db/dbtypes"
	"github.com/planetdecred/pdapi/v6/mempool"
	"github.com/planetdecred/pdapi/v6/pubsub"
	pstypes "github.com/planetdecred/pdapi/v6/pubsub/types"
	"github.com/planetdecred/pdapi/v6/rpcutils"
	"github.com/planetdecred/pdapi/v6/semver"
	"github.com/planetdecred/pdapi/v6/stakedb"

	"github.com/planetdecred/pdapi/cmd/pdapi/api"
	"github.com/planetdecred/pdapi/cmd/pdapi/api/insight"
	"github.com/planetdecred/pdapi/cmd/pdapi/explorer"
	mw "github.com/planetdecred/pdapi/cmd/pdapi/middleware"
	notify "github.com/planetdecred/pdapi/cmd/pdapi/notification"
...
func _main(ctx context.Context) error {
	...
	cfg, err := loadConfig()
	if err != nil {
		fmt.Printf("Failed to load pdapi config: %s\n", err.Error())
		return err
	}
	defer func() {
		if logRotator != nil {
	...
	// dcrd notifies of block 200000 while pdapi has only reached 1000 in
...
func listenAndServeProto(ctx context.Context, wg *sync.WaitGroup, listen, proto string, mux http.Handler) {
	...
	go func() {
		var err error
		if proto == "https" {
			err = server.ListenAndServeTLS("pdapi.cert", "pdapi.key")
		} else {
			err = server.ListenAndServe()
		}
		// If the server dies for any reason other than ErrServerClosed (from
		// graceful server.Shutdown), log the error and request pdapi be
		// shutdown.
